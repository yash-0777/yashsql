...
function joinLines(items) { return (items || []).join('\n'); }
...
export default function EditRecipe() {
  ...
  useEffect(() => {
    api.get(`/recipes/${id}`).then(r => {
      ...
      setIngredients(joinLines(rec.ingredients));
      setInstructions(joinLines(rec.instructions));
      setCategories(joinLines(rec.categories));
    });
  }, [id]);
  ...
  const submit = async e => {
    ...
    try {
      const res = await api.put(`/recipes/${id}`, form, { headers: { 'Content-Type': 'multipart/form-data' } });
